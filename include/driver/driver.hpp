...
class driver {
    ...
    bool validate_arguments() const {
        ...
        if (args.target_hosts.size() != args.target_ports.size()) return false;
        if (args.balancing_algorithm == Algorithm::Type::CONSTANT && (!vm.count("target") || (args.target_idx < 0 || args.target_idx >= args.target_hosts.size())))
            return false;
        return true;
    }
public:
    ...
    int run() {
        ...
        for (std::size_t idx = 0, size = args.target_hosts.size(); idx != size; ++idx) {
            targets.push_back(std::make_pair(std::string_view(args.target_hosts[idx]), std::string_view(args.target_ports[idx])));
        }
        load_balancer::server{}.run(args.listener_host, args.listener_port, AlgorithmInfo { args.balancing_algorithm, args.target_idx }, targets);
